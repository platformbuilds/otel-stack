...
export default function Timeline({ spans }:{ spans: Span[] }){
  ...
  const total = max - min
  function x(ns:number){ return pad + (ns - min) / total * (width - pad - 20) }
        ...
        return (
          <g key={svc} transform={`translate(0,${y})`}>
            ...
            {byService[svc].map(s=>{
              ...
              return (
                <g key={s.spanId} title={`${s.name}`}>
                  <rect x={x1} y={6} width={Math.max(2, x2-x1)} height={laneH-12} fill="#89a" rx={4}/>
                  <text x={x1+4} y={laneH/2} dominantBaseline="middle" style={{fontSize:11, fill:'#fff'}}>{s.name}</text>
                </g>
              )
            })}
          </g>
